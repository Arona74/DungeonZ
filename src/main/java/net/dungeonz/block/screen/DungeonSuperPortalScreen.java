...
public class DungeonSuperPortalScreen extends HandledScreen<DungeonSuperPortalScreenHandler> implements ScreenHandlerListener {
    ...
    public DungeonSuperPortalScreen(DungeonSuperPortalScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void init() {
        ...
        boolean canChangeDungeon = this.handler.getDungeonPortalEntity().getDungeonPlayerCount() == 0
                && !this.handler.getDungeonPortalEntity().isOnCooldown((int) this.client.world.getTime());

        this.changeDungeonButton = this.addDrawableChild(
                ButtonWidget.builder(Text.translatable("text.dungeonz.super_portal.change_dungeon"), button -> {
                    this.client.setScreen(new DungeonSuperPortalSelectionScreen(
                            this.handler.getPos(), this.handler.getDungeonIdList()));
                }).dimensions(this.x + 8, this.y + this.backgroundHeight - 28, 90, 20).build());
        ...
        if (playerIsInDungeonWorld) {
            ...
            List<ItemStack> tickRequiredItems = this.handler.getRequiredItemStacks().get(this.handler.getDungeonPortalEntity().getDifficulty());
            if ((this.handler.getDungeonPortalEntity().getDungeonPlayerUuids().size() + this.handler.getDungeonPortalEntity().getDeadDungeonPlayerUUIDs().size()) < this.handler
                    .getDungeonPortalEntity().getMaxGroupSize() && (tickRequiredItems == null || InventoryHelper.hasRequiredItemStacks(this.playerEntity.getInventory(), tickRequiredItems))
            ...
            if (this.dungeonButton.active && this.privateButton.isEnabled() && !this.handler.getDungeonPortalEntity().getDungeonPlayerUuids().isEmpty()) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Dungeon Timer (top left, dark blue) or Cooldown (dark red)
        if (this.handler.isDungeonTimerActive() && this.handler.getDungeonTimeRemaining() > 0) {
            ...
            String timerText = hours > 0 ? String.format("Run: %dh:%02dm:%02ds", hours, minutes, seconds)
                                         : String.format("Run: %dm:%02ds", minutes, seconds);
        ...
        int activeCount = Math.min(this.handler.getDungeonPortalEntity().getDungeonPlayerUuids().size(), 13);
        ...
        for (int i = 0; i < deadUuids.size() && (activeCount + i) < 13; i++) {
            String deadName = getPlayerName(deadUuids.get(i), 90, 12).getString() + " (" + Text.translatable("text.dungeonz.dead").getString() + ")";
            context.drawText(this.textRenderer, deadName, this.x + 13, k, 0xFF5555, false);
            k += 13;
        }
        // Required items
        ...
        if (this.isPointWithinBounds(230, 6, 20, 18, mouseX, mouseY)) {
            ...
            dungeonInfo.add(Text.translatable("dungeonz.dungeon.info.wind_charge" + (this.handler.isAllowWindCharge() ? "" : ".disabled")));
    ...
    public class DungeonButton extends ButtonWidget {
        ...
        public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            if (!this.active && this.isHovered()) {
                Text text = null;
                if (DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().isOnCooldown((int) DungeonSuperPortalScreen.this.client.world.getTime())) {
                    int cooldown = (DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getCooldownTime() - (int) DungeonSuperPortalScreen.this.client.world.getTime()) / 20;
                ...
                } else if ((DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getDungeonPlayerUuids().size()
                        + DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getDeadDungeonPlayerUUIDs().size()) >= DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity()
                        .getMaxGroupSize()) {
                    text = Text.translatable("text.dungeonz.dungeon_full");
                } else if (client.player != null && !DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getDeadDungeonPlayerUUIDs().isEmpty()
                        && DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getDeadDungeonPlayerUUIDs().contains(client.player.getUuid())) {
                    text = Text.translatable("text.dungeonz.dead_player");
                } else if (!InventoryHelper.hasRequiredItemStacks(client.player.getInventory(), DungeonSuperPortalScreen.this.handler.getRequiredItemStacks().get(DungeonSuperPortalScreen.this.handler.getDungeonPortalEntity().getDifficulty()))) {
                    text = Text.translatable("text.dungeonz.missing");
                } else if (DungeonzMain.isLevelZLoaded) {
                    LevelManager levelManager = ((LevelManagerAccess) DungeonSuperPortalScreen.this.playerEntity).getLevelManager();
                    if (levelManager.getOverallLevel() < DungeonSuperPortalScreen.this.handler.getRequiredLevel()) {
                        text = Text.translatable("text.dungeonz.required_level", DungeonSuperPortalScreen.this.handler.getRequiredLevel());
                    }
                }
                if (text != null) {
