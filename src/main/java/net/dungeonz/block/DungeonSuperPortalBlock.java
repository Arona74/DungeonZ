...
public class DungeonSuperPortalBlock extends DungeonPortalBlock {
    ...
    public static final MapCodec<DungeonSuperPortalBlock> CODEC = DungeonSuperPortalBlock.createCodec(DungeonSuperPortalBlock::new);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (player.getWorld().getBlockEntity(pos) == null
                || !(player.getWorld().getBlockEntity(pos) instanceof DungeonPortalEntity)) {
        ...
        if (player.isCreativeLevelTwoOp() && (superEntity.getDungeon() == null || player.isSneaking())) {
            if (!world.isClient()) {
                DungeonServerPacket.writeS2COpenOpScreenPacket((ServerPlayerEntity) player, superEntity, null);
            }
        ...
        if (superEntity.getDungeonType().isEmpty()) {
            if (!world.isClient()) {
                DungeonServerPacket.writeS2CSuperPortalSelectionPacket((ServerPlayerEntity) player, superEntity);
            }
            return ActionResult.success(world.isClient());
        }
        ...
        if (!world.isClient()) {
            if (DungeonzMain.isPartyAddonLoaded) {
                PartyAddonServerPacket.writeS2CSyncGroupManagerPacket((ServerPlayerEntity) player,
                        ((GroupManagerAccess) player).getGroupManager());
            }
            ExtendedScreenHandlerFactory<?> factory = DungeonSuperPortalEntity.createScreenFactory(superEntity, world, mainPos);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return DungeonGateBlock.validateTicker(type, BlockInit.DUNGEON_SUPER_PORTAL_ENTITY,
                world.isClient() ? DungeonPortalEntity::clientTick : DungeonPortalEntity::serverTick);
    }
    ...
    @Override
    protected MapCodec<? extends DungeonPortalBlock> getCodec() {
        return CODEC;
    }

    public static boolean isOtherSuperPortalBlockNearby(World world, BlockPos pos) {
        return DungeonPortalBlock.isOtherPortalBlockNearby(world, pos, BlockInit.DUNGEON_SUPER_PORTAL);
    }

    @Nullable
    public static BlockPos getMainSuperPortalBlockPos(World world, BlockPos pos) {
        return DungeonPortalBlock.getMainPortalBlockPos(world, pos, BlockInit.DUNGEON_SUPER_PORTAL);
    }
    ...
    public static DungeonSuperPortalEntity getMainSuperPortalEntity(World world, BlockPos pos) {
        ...
        return mainPos != null ? (DungeonSuperPortalEntity) world.getBlockEntity(mainPos) : null;
    }
}
